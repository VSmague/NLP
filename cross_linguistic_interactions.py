...
================================
Builds the cross-linguistic interaction matrix:

    M[i, j] = average over all layers of delta-CE
...
TOP_K_ABLATE = 2   # top-k features of language j to ablate per layer
N_SENTENCES  = 20  # sentences per language to estimate delta-CE
N_CLUSTERS   = 5   # number of clusters for MDS-based clustering

RECOMPUTE = False  # set True to force recomputation
...
print(f"   {n_layers} layers × {n_langs} languages")
print(f"   Layers: {LAYERS}")
print(f"   Estimated duration: ~{n_langs * n_langs * n_layers * N_SENTENCES // 60} min on GPU\n")
...
# =============================================================================
# Build M[i, j] — optimized loop: one SAE loaded once per
# (layer, lang_ablate), evaluated on all lang_eval
# =============================================================================
...
if RECOMPUTE or not os.path.exists(INTERACTION_CACHE):
    ...
    # Pre-tokenize all sentences to avoid repeating the tokenizer
    sentences_per_lang = {
        i: [t for t in multilingual_texts[i*100 : i*100 + N_SENTENCES]]
        for i in range(n_langs)
    }

    print(f"\n[3] Building M[i,j] — average over {n_layers} layers...")
    print(f"   Strategy: load each SAE once → evaluate all languages")
    print(f"   {n_langs}×{n_layers} SAE loads total\n")

    # M_per_layer[i, j, layer_idx] = delta-CE on text_i after ablating features_j at layer
...
# =============================================================================
# Build MDS-based distance matrix (used for clustering in all plots)
# =============================================================================

D_sym = np.zeros((n_langs, n_langs))
for i in range(n_langs):
...
Z_mds = linkage(squareform(D_sym), method='ward')
cluster_labels_arr = fcluster(Z_mds, t=N_CLUSTERS, criterion='maxclust')

clusters = {}
for i, lang in enumerate(LANGUAGES):
    clusters.setdefault(int(cluster_labels_arr[i]), []).append(lang)

print("\nDetected clusters (MDS/Ward, k=5):")
...
ax.set_xlabel("Language whose features are ablated (j)", fontsize=11)
ax.set_ylabel("Evaluated language (i)", fontsize=11)
for i in range(n_langs):
    for j in range(n_langs):
        ax.text(j, i, f"{M[i,j]:+.3f}", ha='center', va='center', fontsize=8,
                color='black' if abs(M[i,j]) < M.max()*0.6 else 'white')
plt.colorbar(im, ax=ax, label=f"Mean ΔCE (avg. {n_layers} layers)")
ax.set_title(f"Cross-linguistic interaction matrix\n"
             f"M[i,j] = mean ΔCE on text i | ablation of features j "
             f"(top-{TOP_K_ABLATE}, avg. {n_layers} layers)",
...
ax.set_title("Normalized matrix — relative impact compared to self-ablation",
             fontsize=12, fontweight='bold', pad=15)
plt.tight_layout()
...
dendrogram(Z_mds, labels=lang_labels, ax=axes[0],
           leaf_font_size=12, color_threshold=0.5*max(Z_mds[:,2]))
axes[0].set_title(f"Hierarchical clustering (Ward, k={N_CLUSTERS}) — avg. layers",
                  fontsize=12, fontweight='bold')
axes[0].set_ylabel("Ward distance")
axes[0].legend(handles=legend_patches, fontsize=7.5, loc='upper right',
               title="Typological family", title_fontsize=8)

order       = dendrogram(Z_mds, no_plot=True)["leaves"]
...
axes[1].set_title("Reordered M", fontsize=11, fontweight='bold')
plt.colorbar(im, ax=axes[1], label="Mean ΔCE")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "3_hierarchical_clustering.png"), dpi=150)
plt.close()
print("→ 3_hierarchical_clustering.png")

# ── Plot 4 — Seaborn clustermap ───────────────────────────────────────────────
...
g = sns.clustermap(df_M, cmap='RdYlGn', figsize=(10, 10),
                   ...
                   linewidths=0.5)
g.ax_heatmap.set_xlabel("Ablated language (j)", fontsize=10)
g.ax_heatmap.set_ylabel("Evaluated language (i)", fontsize=10)
g.fig.suptitle("Clustermap — average over all layers",
               fontsize=11, fontweight='bold', y=1.01)
g.fig.legend(handles=legend_patches, loc='lower left', fontsize=8,
             title="Typological family", title_fontsize=9,
             bbox_to_anchor=(0.0, 0.0))
plt.savefig(os.path.join(OUTPUT_DIR, "4_clustermap.png"), dpi=150, bbox_inches='tight')
plt.close()
print("→ 4_clustermap.png")

# ── Plot 5 — MDS 2D language map with 5 clusters ─────────────────────────────
fig, ax = plt.subplots(figsize=(9, 7))

for i, lang in enumerate(LANGUAGES):
    family = TYPO_FAMILIES.get(lang, '')
    color  = FAMILY_COLORS.get(family, '#888')
    ax.scatter(coords[i, 0], coords[i, 1], s=220, color=color,
...
# Cluster circles
for c in sorted(np.unique(cluster_labels_arr)):
    idx    = np.where(cluster_labels_arr == c)[0]
    ...
    circle = plt.Circle((cx, cy), radius, fill=False,
                         linestyle='--', linewidth=1.5, color='grey', alpha=0.5)
    ax.add_patch(circle)
    langs_in_c = [LANGUAGES[k].upper() for k in idx]
    ax.text(cx, cy - radius - 0.03, f"Cluster {c}",
            ha='center', fontsize=9, color='grey', style='italic')

ax.legend(handles=legend_patches, fontsize=8, loc='upper right',
          title="Typological family")
ax.set_title(f"MDS language map — mutual influence space\n"
             f"(proximity = similar interaction profile, k={N_CLUSTERS} clusters)",
             fontsize=12, fontweight='bold')
ax.set_xlabel("Dim 1")
ax.set_ylabel("Dim 2")
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "5_mds_cluster_map.png"), dpi=150)
plt.close()
print("→ 5_mds_cluster_map.png")

# ── Plot 6 — Delta-CE per layer for selected pairs ────────────────────────────
if M_per_layer is not None:
    ...
    for ax, (lang_eval, lang_ablate) in zip(axes, interesting_pairs):
        ...
        ax.axhline(float(M[i, j]), color='tomato', linestyle='--',
                   linewidth=1.2, label=f"Mean = {M[i,j]:.3f}")
    ...
    fig.suptitle("ΔCE evolution per layer for selected pairs",
...
axes[0].set_ylabel("Mean ΔCE")
axes[0].set_title("Top-15 most asymmetric pairs", fontweight='bold')
axes[0].legend()
...
axes[1].plot([0, max_val], [0, max_val], 'k--', alpha=0.4, label="Perfect symmetry")
...
axes[1].set_title("Symmetry of influence between languages", fontweight='bold')
axes[1].legend()
axes[1].grid(linestyle='--', alpha=0.4)
plt.suptitle("Asymmetry analysis (average over all layers)",
             fontsize=12, fontweight='bold')
plt.tight_layout()
