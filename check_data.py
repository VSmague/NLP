...
# Afficher les métadonnées des premières lignes
print("=== 15 premières entrées ===")
for i, line in enumerate(lines[:15]):
    obj = json.loads(line)
    print(f"  [{i:3d}] {obj}")

# Vérifier si un champ 'lan' existe
print("\n=== Champs disponibles ===")
sample = json.loads(lines[0])
print(f"  Clés : {list(sample.keys())}")

# Si un champ langue existe, compter par bloc de 100
if 'lan' in json.loads(lines[0]):
    ...
    for i, lan in enumerate(TARGET_LANGUAGES):
        ...
        match = "✅" if counts.most_common(1)[0][0] == lan else "❌"
        print(f"  Bloc {i} (attendu: {lan}) → {dict(counts)} {match}")
else:
    print("\n⚠️  Pas de champ 'lang' détecté — vérification manuelle nécessaire")
